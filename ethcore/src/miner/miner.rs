...
pub struct Miner {
	...
	next_mandatory_reseal: Mutex<Instant>,
...
impl Miner {
	...
	fn new_raw(options: MinerOptions, gas_pricer: GasPricer, spec: &Spec, accounts: Option<Arc<AccountProvider>>) -> Miner {
		...
		Miner {
			...
			next_mandatory_reseal: Mutex::new(Instant::now() + options.reseal_max_period),
	...
	fn forced_sealing(&self) -> bool {
		self.options.force_sealing
			...
			|| Instant::now() > *self.next_mandatory_reseal.lock()
	}
...
*/
	...
	fn seal_and_import_block_internally(&self, chain: &MiningBlockChainClient, block: ClosedBlock) -> bool {
		if !block.transactions().is_empty() || self.forced_sealing() {
			*self.next_mandatory_reseal.lock() = Instant::now() + self.options.reseal_max_period;
